...
* Fix method missing again.. put on Object?
* Rename sym_from_switch to switch_to_sym
* Fix issue with having command name in the args list, #args_without_command
* Fix annoying 'Program x required' message
* Fix bool switches --[no-]-foo ... document
* Specs / mocking for passing a class to #when_called, to be initialized with or without a method / refactor with matchers etc
* Option defaults when null
* Refactor specs / improve testing capabilities
* Non-greedy matching of command name
* 1.9.x compatability
* Publish RDoc on mini page / screencasts
* Add compat.rb with warnings if needed if any api changes appear
...
* Change  VerboseFileUtils to output basename only ? 
